...
@section('page-script')
  <script>
    ...
    const listNews = @json($listNews);
    console.log('---', listNews);
    const defaultThumbnail = 'assets/img/ar_SA.png'; // Replace with your default image URL
        ...
        checkImageUrl(news.thumbnail).then(isValid => {
          ...
          $('#data-table-body').append(`
            <tr>
              ...
              <td>${news.alias}</td>
              <td>${news?.author?.name || 'Chưa có'}</td>
              <td>${Math.round(news?.rating * 100) / 100 || 'Chưa có'}</td>
              <td>${Math.round(news?.rating_count) || 'Chưa có'}</td>
              <td>${formatDate(news.created_at)}</td>
              <td>${formatDate(news.update_at)}</td>
            </tr>
          `);
...
<table class="table dt-fixedcolumns">
    <thead>
        <tr>
            ...
            <th class="m-0 p-2" style="font-weight: 600;">Rating</th>
            <th class="m-0 p-2" style="font-weight: 600;">Rating Count</th>
